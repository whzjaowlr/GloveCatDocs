# GloveCat Terms of Service

> **Last Updated**: 2026-01-06  
> **Effective Date**: 2026-01-06

---

## 1. Acceptance of Terms

By accessing, using, or interacting with GloveCat token ($GCAT), smart contracts, website, or any
related services (collectively, the "Services"), you agree to be bound by these Terms of Service
("Terms").

**If you do not agree to these Terms, do not use the Services.**

---

## 2. Eligibility and Restricted Jurisdictions

### 2.1 Age Requirement

You must be at least 18 years old (or the age of legal majority in your jurisdiction) to use the
Services.

### 2.2 RESTRICTED JURISDICTIONS

> [!CAUTION] **THE SERVICES ARE NOT AVAILABLE TO RESIDENTS OF THE FOLLOWING JURISDICTIONS:**

**You are NOT permitted to use the Services if you are a citizen, resident, or located in:**

1. **Republic of Korea (South Korea)**
2. **Democratic People's Republic of Korea (North Korea)**
3. **Iran**
4. **Syria**
5. **Cuba**
6. **Crimea region of Ukraine**
7. **Any other jurisdiction where cryptocurrency or digital assets are prohibited by law**

### 2.3 User Representation

By using the Services, you represent and warrant that:

- You are NOT a citizen or resident of any Restricted Jurisdiction
- You are NOT currently located in any Restricted Jurisdiction
- You are NOT using VPN, proxy, or other means to circumvent geographic restrictions
- You are NOT on any government sanctions list (OFAC, UN, EU, etc.)

### 2.4 Verification

We reserve the right to implement geographic verification measures at any time. Users who are
determined to be from Restricted Jurisdictions may have their access terminated without notice.

---

## 3. Nature of Services

### 3.1 Not Financial Advice

The Services do not constitute:

- Investment advice
- Financial advice
- Tax advice
- Legal advice

### 3.2 No Guarantees

We make **NO guarantees** regarding:

- Token price or value
- Future returns or profits
- Token utility or functionality
- Exchange listings

### 3.3 Decentralized Protocol

GloveCat is a decentralized protocol. Once deployed, smart contracts operate autonomously on the
blockchain. We do not have the ability to:

- Reverse transactions
- Recover lost tokens
- Modify immutable contract code

---

### 4.1 Token Tax Structure

Transactions may be subject to automatic protocol-level tax:

| Type          | Rate | Purpose                                                |
| ------------- | ---- | ------------------------------------------------------ |
| Liquidity Tax | 1%   | Automatically added to on-chain liquidity              |
| Ecosystem Tax | 1%   | Protocol-level allocation for ecosystem sustainability |
| Burn Tax      | 0%   | Irreversibly removed from total supply                 |

> **Important**: These are protocol-level allocations automatically executed by smart contracts. No
> single party can unilaterally access or withdraw these funds.

### 4.2 Fund Management

- Protocol funds are managed by a multisignature wallet
- No single party can unilaterally withdraw funds
- Funds are used for ecosystem development

---

## 5. Risks

### 5.1 Investment Risks

- **Total Loss**: You may lose 100% of your investment
- **Volatility**: Token price may fluctuate significantly
- **Liquidity**: You may not be able to sell tokens at desired price

### 5.2 Technical Risks

- **Smart Contract Risk**: Bugs or vulnerabilities may exist
- **Blockchain Risk**: Network congestion, forks, or failures
- **Wallet Risk**: Loss of private keys results in permanent loss

### 5.3 MEV (Maximal Extractable Value) Risks

- **Sandwich Attack**: Your trades may be front-run by MEV bots
- **Price Impact**: Large trades may result in unfavorable execution prices

> [!TIP] **MEV Protection Recommendation**
>
> To protect your transactions from MEV attacks:
>
> 1. **Enable UniswapX** in Uniswap settings (⚙️) - Recommended
> 2. Or use Flashbots RPC: `https://rpc.flashbots.net/base`
>
> UniswapX provides automatic MEV protection and may return value to you.

### 5.4 Regulatory Risks

- **Legal Changes**: Future regulations may affect token legality
- **Tax Obligations**: You are responsible for tax compliance in your jurisdiction

### 5.5 Trading Limits

The protocol implements daily volume limits that reset every 24 hours from your last transaction:

- **Daily Volume Limit**: Maximum daily trading volume per wallet
- **Reset Timing**: Limits reset 24 hours after your first daily transaction
- **Note**: Timing of limit resets depends on your individual transaction history

---

## 6. Prohibited Activities

You agree NOT to:

1. Engage in market manipulation
2. Use the Services for money laundering or illegal activities
3. Provide false information about your identity or location
4. Interfere with the proper functioning of the Services

---

## 7. Disclaimers

### 7.1 "AS IS" Basis

THE SERVICES ARE PROVIDED "AS IS" AND "AS AVAILABLE" WITHOUT WARRANTIES OF ANY KIND, EXPRESS OR
IMPLIED.

### 7.2 Limitation of Liability

TO THE MAXIMUM EXTENT PERMITTED BY LAW, WE SHALL NOT BE LIABLE FOR:

- Any loss of profits, data, or digital assets
- Any indirect, incidental, or consequential damages
- Any damages arising from your use of the Services

---

## 8. Indemnification

You agree to indemnify and hold harmless GloveCat, its founders, developers, and contributors from
any claims, damages, or expenses arising from:

- Your use of the Services
- Your violation of these Terms
- Your violation of any applicable laws

---

## 9. Governing Law

These Terms shall be governed by and construed in accordance with the laws of **Republic of Korea**, without regard to its conflict of law provisions.

---

## 10. Dispute Resolution

Any disputes arising from these Terms shall be resolved through binding arbitration, not in court.
Class action lawsuits are waived.

---

## 11. Changes to Terms

We reserve the right to modify these Terms at any time. Continued use of the Services after changes
constitutes acceptance of the new Terms.

---

## 12. Contact

For questions about these Terms, contact: [glovecatcoin@gmail.com](mailto:glovecatcoin@gmail.com)

---

## 13. Severability

If any provision of these Terms is found to be unenforceable, the remaining provisions shall
continue in full force and effect.

---

**BY USING GLOVECAT SERVICES, YOU ACKNOWLEDGE THAT YOU HAVE READ, UNDERSTOOD, AND AGREE TO BE BOUND
BY THESE TERMS OF SERVICE.**

---

> [!WARNING] **대한민국 거주자 이용 불가**
>
> 본 서비스는 대한민국 거주자에게 제공되지 않습니다. 대한민국 거주자의 서비스 이용은 엄격히
> 금지되며, 이를 위반하여 발생하는 모든 법적 책임은 이용자 본인에게 있습니다.

---
